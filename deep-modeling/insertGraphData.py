...
def insert_httpreqs(graph, reqlist, projname, session, user, logger=None):
    ...
    prev_hreq_n = None
    for hreq in reqlist:
        logger.info("Processing HTTP Request ID {} / {}".format(hreq[0],
                                                                len(reqlist)))
...
def insert_cmd2http(graph, idlist, projname, session, user, logger=None):
 ...
 HTTP request ID {}  / {}".format(cmdid, rid, len(idlist)))
...
HTTP request ID {} / {}".format(q_id, hreq_id, len(queries)))
