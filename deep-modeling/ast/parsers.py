...
def headers_to_list(hdrs):
    ...
    hdrs = hdrs.split("\r\n")
    hdrs = [(hdr.split(":", 1)[0], hdr.split(":", 1)[1]) for hdr in hdrs] # from list of strings to  list of (k,v)
    return hdrs
...
def parse_selcmd(command, target, value, seq, ts, projname, session, user):
    cmd_n = SeleneseCommand(projname, session, user, seq, ts, command, target, value)
